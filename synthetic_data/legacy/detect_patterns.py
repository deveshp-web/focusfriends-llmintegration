...
def compute_flag_id(student_id, streaks):
    """Stable id for 'this student + this exact set of findings'.

    Used to remember teacher review actions (see review_flags.py /
    flag_review_store.py) across re-runs of this script. Two runs produce
    the same id only if the streaks are identical (same type/span/length) -
    a genuinely new episode gets a new id on purpose, so dismissing last
    week's negative streak never silently dismisses a brand new one that
    starts today.
    """
    fingerprint = "|".join(sorted(
        f"{s['type']}:{s['start_ts']}:{s['end_ts']}:{s['length']}" for s in streaks
    ))
